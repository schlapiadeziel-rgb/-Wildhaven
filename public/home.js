...
export function installHome(Game, BUILDS, ITEMS, height, dist) {
  ...
  const edgeSupportedBy = (g,x,z,angle) => {
    const horizontal = Math.abs(Math.cos(edgeAngle(angle))) > .5;
    return g.buildings.filter(b=>b.type==="floor" && mine(g,b)).some(f=> horizontal
      ? Math.abs(f.x-x)<.12 && Math.abs(Math.abs(f.z-z)-1.5)<.12
      : Math.abs(f.z-z)<.12 && Math.abs(Math.abs(f.x-x)-1.5)<.12);
  };
  ...
  Game.prototype.snapBuild = function(type,x,z,angle=0) {
    ...
    // Pick a real floor edge, not just a mathematical grid. The requested
    // direction is strict: "横向" only evaluates the north/south edges and
    // "竖向" only evaluates east/west edges. This prevents auto-snap from
    // silently changing the direction selected by the player.
    const choices=[];
  ...
  Game.prototype.canBuild = function(type,x,z,angle=0) {
    ...
    if(["wall","door"].includes(type) && !edgeSupportedBy(this,x,z,angle)) return false;
    if(["roof","rug"].includes(type) && !supporting.some(b=>mine(this,b))) return false;
    if(type==="floor") {
